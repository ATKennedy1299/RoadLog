...
dependencies {
    ...
    // sign-in fix calls for — 1.3.0 still only needs compileSdk 34. googleid
    // is pinned down to 1.1.1 to match: 1.2.1 transitively pulls in
    // credentials 1.6.0+ itself, which silently overrode the explicit
    // 1.3.0 pin above (Gradle resolves conflicting transitive versions to
    // the highest one requested by default) — this is what actually
    // caused the previous attempt to still fail on 1.6.0's requirements.
